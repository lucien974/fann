...
#define fann_relu_derive(steepness, sum) (steepness > 0.0 ? steepness : 0.0)

/* FANN_LEAKY_RELU */
#define fann_leaky_relu_real(sum) (sum > 0.0 ? sum : (sum / 100.0))
#define fann_leaky_relu_derive(steepness, sum) (steepness > 0.0 ? steepness : (steepness / 100.0))
...
{ \
	...
	case FANN_LEAKY_RELU: 
		result = (fann_type)fann_leaky_relu(value);
		break;
}
